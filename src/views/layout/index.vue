<template>
  <n-layout has-sider v-resize @resize="handleResize">
    ...
    >
      ...
      <div class="content-bottom" v-if="!collapsed">
        {{byteConvert(aboutInfo?.quota.usage)}} / {{byteConvert(aboutInfo?.quota.limit)}} <n-text type="primary" @click="showCode = true">會員碼</n-text>
        <n-progress 
  ...
  <n-modal v-model:show="showCode">
    <n-card style="width: 600px;" title="會員碼">
      <template #header-extra>
        <n-icon @click="showCode = false">
          <circle-x></circle-x>
        </n-icon>
      </template>
      <n-input placeholder="會員碼" v-model:value="code"></n-input>
