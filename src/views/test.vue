    ...
    <n-upload ref="uploadRef" :show-retry-button="false" @remove="moveFile" multiple :show-file-list="false" v-model:file-list="fileListRef" :default-upload="false" @change="changeFile">
        <n-upload-dragger style="width: 100vw; height: 100vh;">
          ...
          <template v-else>
            ...
            <n-text style="font-size: 16px;">點擊或者拖動文件到該區域來上傳</n-text>
            <n-text type="info" style="margin: 8px 0 0 0;">
              支持連結上拼上id傳到指定文件夾哦~(/t/12154sds)
            </n-text>  
            <n-p depth="3" style="margin: 8px 0 0 0;">
              請不要上傳敏感數據，比如你的銀行卡號和密碼，信用卡號有效期和安全碼
            </n-p>
  ...
  const postFile = async () => {
    ...
    if(!gcidItem) {
      ...
      gcidItem = await gcid(fileInfo.file, Comlink.proxy({
        progress: (p:number) => {
          if(fileInfo) {
            if(fileInfo.name.indexOf('-正在效驗文件') === -1) {
              fileInfo.name = fileInfo.name + '-正在效驗文件'
            }
    ...
    fileInfo.name = fileInfo.name.replace('-正在效驗文件', '').replace('-正在上傳文件', '')
          ...
          try {
            const ossPrarams:any = {
              ...
              progress: (p:number, checkpoint:object) => {
                if(fileInfo) {
                  if(fileInfo.name.indexOf('-正在上傳文件') === -1) {
                     fileInfo.name = fileInfo.name.replace('-正在效驗文件', '') + '-正在上傳文件'
                  }
                  fileInfo.status = 'uploading'
                  fileInfo.percentage = p * 100
                }
            ...
            if((fileUpload.value[fileInfo.id].retry || 0) < 3) {
              window.$message.error('稍後會重試' + fileInfo.name)
            }
            postFile()
          }
      } else {
