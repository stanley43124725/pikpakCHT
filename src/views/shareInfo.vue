<template>
  <n-spin :show="loading " description="獲取中">
    <div class="share-info-page" >
      <div class="header" v-if="shareInfo?.share_status === 'OK'">
        <div class="title n-ellipsis">
          ...
          <n-text  depth="3" style="font-size: 12px">
            失效時間： {{shareInfo?.expiration_left === '-1' ? '永久有效' : (shareInfo?.expiration_left + '天')}} 失效次數：{{shareInfo?.restore_count_left === '-1' ? '無限次' : (shareInfo?.restore_count_left + '次')}}
          </n-text>
        </div>
        <div class="action">
          <n-button type="primary" @click="restore">{{checkedRowKeys.length ? '轉存已選' : '轉存全部'}}</n-button>
        </div>
      </div>
      <template v-if="shareInfo?.share_status === 'PASS_CODE_EMPTY' || shareInfo?.share_status === 'PASS_CODE_ERROR'">
        <div class="share-password">
          ...
          <n-form class="password-form">
            <n-form-item label="請輸入提取碼" >
              <n-input-group>
                <n-input v-model:value="password" type="password" show-password-on="mousedown" @keyup.enter="getInfo" placeholder="請輸入提取碼"></n-input>
      ...
      <template v-else-if="shareInfo?.share_status === 'DELETED'">
        <div class="share-file-box" style="height: 100vh">
          <n-empty description="抱歉，該分享已被作者刪除">
            <template #icon>
            <n-icon>
          ...
          <template #extra>
            <n-button size="small" @click="goHome">去首頁</n-button>
          </template>
        </n-empty>
        </div>
      </template>
...
const restore = () => {
  ...
  http.post('https://api-drive.mypikpak.com/drive/v1/share/restore', {
    ...
    .then(res => {
      window.$message.success('轉存成功，可能需要等待一段時間')
