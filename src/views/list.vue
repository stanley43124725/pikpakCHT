<template>
  <div class="list-page list-page-files" >
      ...
      <div class="action">
        <n-space>
          <n-button type="default" @click="movePost" v-if="moveFiles?.length">
            黏貼已剪切{{moveFiles.length}}項資源
          </n-button>
          <n-button v-if="copyFiles?.length" @click="copyPost">
            黏貼已複製{{copyFiles.length}}項資源
          </n-button>
    ...
    <n-modal v-model:show="showAddUrl">
      <n-card style="width: 600px;" title="添加連結或新建文件夾">
        <template #header-extra>
          <n-icon @click="showAddUrl = false">
            <circle-x></circle-x>
          </n-icon>
        </template>
        <n-alert :show-icon="false" closable title="添加說明">
          <div>1.支持Magnet連結(magnet:?xt=urn)，Magent連結只能默認保存到My Pack</div>
          <div>2.支持秒傳連結(PikPak://PikPak Tutorial.mp4|19682618|123)秒傳連結默認保存到當前文件夾或第一個文件夾不能保存到根目錄</div>
          <div>3.支持新建文件夾（普通格式，不帶:）</div>
          <div>4.換行添加多個</div>
        </n-alert>
        <br />
        <n-input type="textarea" :rows="4" placeholder="請按說明填寫" v-model:value="newUrl"></n-input>
        <template #action>
    ...
    <n-modal v-model:show="showVideo">
      <n-card style="width: 100vw; height: 100vh;" :title="fileInfo ? fileInfo.name : '影片'">
        <template #header-extra>
    ...
    <n-modal v-model:show="showImage">
      <n-card style="width: 100vw; height: 100vh;" :title="fileInfo ? fileInfo.name : '圖片'">
        <template #header-extra>
    ...
    <n-modal v-model:show="showName">
      <n-card style="width: 600px;" title="修改名稱">
        <template #header-extra>
    ...
    <n-modal v-model:show="showUserMenu">
      <n-card style="width: 600px;" title="自訂菜單">
        <template #header>
          自訂菜單 <a href="https://www.tjsky.net/?p=220#i-8" target="_blank"> <n-icon style="vertical-align: middle;" size="20" color="#d03050"><zoom-question></zoom-question></n-icon> </a>
        </template>
        <template #header-extra>
    ...
    <n-modal v-model:show="showCopy">
      <n-card style="width: 600px;" title="複製連結">
        <template #header-extra>
          ...
          <template v-for="item in fileInfo?.medias" :key="item.media_id">
            <n-form-item :label="item.media_name">
              <n-input-group>
                ...
                <n-button type="primary" @click="copy(item.link.url)">複製</n-button>
              </n-input-group>
            </n-form-item>
          </template>
          <n-form-item label="連結">
            <n-input-group>
              <n-input :value="fileInfo?.web_content_link"></n-input>
              <n-button type="primary" @click="copy(fileInfo.web_content_link)">複製</n-button>
            </n-input-group>
          </n-form-item>
        </n-form>
      </n-card>
    ...
    <n-modal v-model:show="showCopyFail">
      <n-card style="width: 600px;" title="複製失敗，自己選擇複製">
        <template #header-extra>
...
import axios from 'axios';
  ...
  const copy = (value:string) => {
    nextTick(() => {
      ...
      clipboard.on('error', (e) => {
        window.$message.error('複製失敗，您可以F12打開控制台手動複製，或手動複製跳出視窗輸入框')
  ...
  const getAllFile = async (title?:string) => {
    ...
    nRef.value.content = '共獲取到' + downFileList.value.length + '個文件'
  }
    ...
    getFile(sharePikpak.value.id)
      .then((res:any) => {
        ...
        .then((res:any) => {
          ...
          copy(sharePikPakUrl.value + (sharePikPakPassword.value ? (' 提取密碼  ' + sharePikPakPassword.value) : ''))
        }).catch(res => {
          window.$message.error(res.response.error || '出錯了')
        })
        .finally(() => {
