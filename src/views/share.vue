<template>
  <div class="list-page">
    <n-alert title="感謝@shabitt" type="info" closable>
      <n-space>
    ...
    <br />
    <n-alert title="請先設置默認上傳目錄" type="error" v-if="!uploadFolder || !uploadFolder.id">
      請先在文件列表操作選擇設置默認 <a href="https://www.tjsky.net/?p=220#i-4" target="_blank"> <n-icon style="vertical-align: middle;" size="20" color="#d03050"><zoom-question></zoom-question></n-icon> </a>
    </n-alert>
    
    <n-alert title="默認目錄" type="success" v-else>
      {{uploadFolder.name}}
    </n-alert>
    <br />
    ...
    if(columns.value.length == 0) {
      if(list.value.length) {
        for(let k in list.value[0].properties) {
          if(k !== '發布人' && k !== '發布日期' && k !== '分類' && k !== '標籤') {
            columns.value.push({
              title: k,
              key: k,
              ellipsis: {
                tooltip: {
                  placement: k === '名稱' ? 'right' : 'bottom'
                },
              },
              width: k === '大小' ? 100 : (k === '名稱' ? 1000 : undefined),
              render: (row:any) => {
      ...
      columns.value.push({
        ...
        render: (row:any) => h(NText, {
          type: 'primary',
          onClick: () => {
            addUrl(row.properties['連結'].rich_text[0].plain_text)
          }
        }, {default: () => '保存'})
      })
    }
...
const addUrl = (url:string) => {
  ...
  if(!getUploadFolder().id) {
    window.$message.error('請先設置默認上傳目錄')
    return false
  }
